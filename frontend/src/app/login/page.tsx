...
import { loginUser, getAuthProfile } from '@/lib/api'
...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Capturar token de la URL si se inicia sesión vía OAuth
    ...
    if (token) {
      ...
      getAuthProfile()
        ...
        .catch(err => {
          console.error(err)
          setError('No se pudo verificar la sesión social.')
          setLoading(false)
        })
    }
  }, [login, router])
  ...
  return (
    <main className="min-h-screen pt-0 pb-24 flex items-center justify-center bg-surface-container-lowest relative overflow-hidden">
      ...
      <div className="w-full max-w-md px-6 relative z-10 text-reveal">
        <div className="bg-white/80 backdrop-blur-xl border border-white/50 shadow-2xl rounded-4xl p-10">
          <div className="text-center mb-10">
            ...
            <p className="text-on-surface-variant font-medium">
              Accede a tu cuenta para gestionar tus reservas médicas.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full luminous-gradient text-white py-4 rounded-2xl font-bold text-lg shadow-[0_10px_20px_rgba(2,105,106,0.2)] hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center disabled:opacity-70"
            >
              {loading ? (
